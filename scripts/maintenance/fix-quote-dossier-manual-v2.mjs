...
function patchRoute() {
  ...
  // Remove qualquer observação automática de pronto pagamento/condição de pagamento.
  source = source.replace(/\n\s*if\s*\(\/pronto\\s\+pagamento[\s\S]*?observations\.push\([^)]*pagamento[^)]*\);\s*\n\s*\}/m, "\n");

  source = source.replace(
    /if \(!observations\.length\) \{[\s\S]*?observations\.push\("Rever fotografias, descrição, características e medidas antes de entregar ao cliente\."\);[\s\S]*?\}/m,
...
function patchParser() {
  ...
  if (/function extractCustomer\(text = ""\) \{[\s\S]*?\n\}\n\nfunction extractTotal/m.test(source)) {
    source = source.replace(/function extractCustomer\(text = ""\) \{[\s\S]*?\n\}\n\nfunction extractTotal/m, `${robustCustomerFunction}\n\nfunction extractTotal`);
  } else {
    console.warn("⚠️ Não encontrei extractCustomer para substituir. Parser não alterado nessa parte.");
  }
...
console.log("✅ Correções manuais v2 aplicadas.");
console.log("Corrige:");
console.log(" - remove observações de pronto pagamento/condição de pagamento");
console.log(" - PDF sem páginas em branco causadas por header/footer");
console.log(" - cliente extraído acima ou abaixo do bloco Exmo.(s) Sr.(s)");
console.log("");
