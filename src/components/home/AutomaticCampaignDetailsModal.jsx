...
function getPvpUpdateCodes(campanha) {
  const raw = campanha?.pdfs?.pvpUpdateCodes;
...
export default function AutomaticCampaignDetailsModal({
  ...
  const pvpUpdateCodes = getPvpUpdateCodes(campanha);

  async function copiarCodigosAtualizacaoPvp() {
    if (!pvpUpdateCodes.length) return;

    try {
      await navigator.clipboard.writeText(pvpUpdateCodes.join("|"));
      setCodesCopied(true);
        ...
        <div className="ai-popup-scroll">
          ...
          {pvpUpdateCodes.length ? (
            <div className="automatic-campaign-pvp-update">
              ...
              <code>{pvpUpdateCodes.join("|")}</code>
              <button
                type="button"
                className="btn btn-secondary"
                onClick={copiarCodigosAtualizacaoPvp}
              >
